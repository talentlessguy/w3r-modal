...
```ts
import { initializeConnector as init } from '@web3-react/core'
import { MetaMask } from '@web3-react/metamask'
import { WalletConnect } from '@web3-react/walletconnect'
import type { Connector } from '@web3-react/types'
import type { Web3ReactHooks } from '@web3-react/core'

export const [metaMask, metaMaskHooks] = init<MetaMask>((actions) => new MetaMask(actions), [1])

export const [walletConnect, walletConnectHooks] = init<WalletConnect>(
  (actions) =>
  ...
  return (
    <>
      <style jsx global>
        {`
          :root {
            ${cssStringFromTheme(defaultTheme)}
          }
        `}
      </style>
      <Component {...pageProps} />
